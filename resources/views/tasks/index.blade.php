...
@section('content')
    <div class="container mx-auto my-10 px-6 lg:px-12">
        <h1 class="text-3xl font-bold text-white mb-6">Задачи</h1>
        ...
        <form method="GET" action="{{ route('tasks.index') }}" class="mb-6">
            <div class="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 items-center">
                ...
                <label for="task_type" class="text-white">Тип задачи:</label>
                <select name="task_type" id="task_type" class="px-4 py-2 bg-gray-800 text-white rounded-md">
                ...
                <!-- Фильтр по дедлайну -->
                <label for="deadline_sort" class="text-white">Сортировка по дедлайну:</label>
                <select name="deadline_sort" id="deadline_sort" class="px-4 py-2 bg-gray-800 text-white rounded-md">
        ...
        @if (session('success'))
            <div class="alert alert-success mt-3 p-4 bg-green-500 text-white rounded-md">
                {{ session('success') }}
            </div>
        @elseif (session('error'))
            <div class="alert alert-danger mt-3 p-4 bg-red-500 text-white rounded-md">
                {{ session('error') }}
            </div>
        @endif
        ...
        <ul class="list-none mt-6 space-y-4">
            @foreach ($tasks as $task)
                <li class="p-4 border border-gray-700 bg-gray-800 rounded-md">
                    <strong class="text-lg text-white">{{ $task->title }}</strong><br>
                    <p class="text-gray-300">{{ $task->description }}</p><br>
                    <small class="text-gray-500">Дедлайн: {{ $task->deadline }}</small><br>

                    <!-- Отображение команды, если задача привязана к команде -->
                    @if ($task->team)
                        <p class="text-gray-400 mt-2">Команда: {{ $task->team->name }}</p>
                    @endif
